  ...
  googleClient = function(keys) {
    ...
    this.requestFunctions = {
      contacts: function(tokens, cb) {
        return oauth.get('https://www.google.com/m8/feeds/contacts/default/full?alt=json', tokens.access_token, tokens.access_token_secret, function(err, data, res) {
          ...
          return async.map(data.feed.entry, function(entry, cb) {
            ...
            contact = {
              entry: entry,
              fullname: entry['gd$name'] ? entry['gd$name']['gd$fullName']['$t'] : null
            };
